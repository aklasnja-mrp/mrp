...
qrels = 'reference_tables/qrels.robust04.txt'

searcher = SimpleSearcher('/data/anserini/lucene-index.robust04.pos+docvectors+rawdocs')

d = pd.read_csv('reference_tables/topics.robust04.bm25.map.dataset.txt')
...
for i, l in enumerate(open(qrels)):
    ...
    if score > 0:        
        ...
        try:
            ...
            failed = ds + '_failed_qrels.txt'
            with open(os.path.join('data',ds,'notes',failed), 'a') as out:
                out.write(f'failed: {qid} {rank} {docid} {score}\n\n')
                print(f'failed {qid} {rank} {docid}')
                continue
#--------------------------------------------------------------------
#check counts 
